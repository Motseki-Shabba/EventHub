...
function EventsDisplay() {
  ...
  const { getAllEvents, purchaseTickets, resetStateFlags } = useEventActions();
  ...
  useEffect(() => {
    ...
    const ticketQuantity = searchParams.get("quantity") || "1";
    ...
    if (paymentStatus === "success" && eventId && ticketId) {
      ...
      if (event && ticket) {
        ...
        // Process the ticket purchase through our API
        ...
        purchaseTickets(purchaseRequest)
          .then(() => {
            // Payment successfully processed in our system
          ...
          .catch((error) => {
            api.error({
              ...
              description:
                error.message ||
                "Failed to process your ticket purchase in our system.",
            });
          })
          .finally(() => {
            setIsProcessingPayment(false);
            // Replace URL to remove query parameters
            router.replace("/AttendeeManager/ViewEvents", { scroll: false });
          });
      }
    } else if (paymentStatus === "cancelled") {
      api.info({
        message: "Payment Cancelled",
        description: "Your payment was cancelled. No tickets were purchased.",
      });
      router.replace("/AttendeeManager/ViewEvents", { scroll: false });
    }
  }, [searchParams, events]);
  ...
  const handleBuyTicket = (ticket: ITicket) => {
    if (!ticket.remainingQuantity || ticket.remainingQuantity <= 0) {
      api.error({
        message: "Sold Out",
        description: "Sorry, this ticket is sold out!",
      });
      return;
    }
